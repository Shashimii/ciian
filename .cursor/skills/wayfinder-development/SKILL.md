...
```
For form helpers, `--with-form` is required in this app (see `vite.config.ts` `formVariants: true`).
### Import Patterns
...
1. Run `php artisan wayfinder:generate --with-form --no-interaction` to regenerate routes if Vite plugin isn't installed (this app always needs `--with-form` because forms use `.form()`)
