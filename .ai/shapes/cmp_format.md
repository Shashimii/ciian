...
>
> **Exists:** the `ciian_cmp` table, `App\Models\Ciian\Component\Component` (with `definition()`), `Database\Seeders\CiianComponentSeeder` seeding the Button block, `ComponentFactory`, the `components.manage` permission, a read-only index at `/admin/components` (`resources/js/pages/component/index.tsx`), and the `resources/js/components/` split into `core/` (internal app UI), `default/` (shipped blocks), `custom/` (uploads) and `ui/` (primitives), with `default/button.tsx` checked in.
>
...
- `resources/js/components/default/` — shipped blocks (`custom/` for uploads, `core/` for internal app UI). See `.ai/rules/design.md`.
...
**`component`** (optional)  
Module path used when a real TSX file exists on disk, e.g. `"@/components/default/button"`.
